...
> - Stackable: You can stack services vertically and on-the-fly. 
 ...
 - 轻量级：容器利用并共享主机内核。
 - 可替换：您可以在保持运行时(on-the-fly)部署更新和升级。
...
> A container runs natively on Linux and shares the kernel of the host machine with other containers. It runs a discrete process, taking no more memory than any other executable, making it lightweight. 
 ...
 容器在Linux上本地运行，并与其他容器共享主机的内核。它运行一个离散的进程，不需要比任何其他可执行文件占用更多的内存，因此它是轻量级的。 
 
> By contrast, a virtual machine (VM) runs a full-blown “guest” operating system with virtual access to host resources through a hypervisor. In general, VMs provide an environment with more resources than most applications need. 
 
 相反，虚拟机(VM)运行一个完整的“游客”操作系统，通过管理程序虚拟访问主机资源。一般来说，VM提供的环境比大多数应用程序所需的资源更多。 
...
> Install a maintained version of Docker Community Edition (CE) or Enterprise Edition (EE) on a supported platform . 
 ...
 在受支持的平台上安装Docker 社区版(CE)或企业版(EE)的维护的版本。 
...
> 1. Run `docker --version` and ensure that you have a supported version of Docker:
 ...
 运行`docker --version`并确定您有一个受支持的Docker版本： 
 ```
 $ docker --version
...
> 2. Run `docker info` or (`docker version` without `--`) to view even more details about your docker installation:
 ...
 运行`docker info`(`docker version`没有`--`)来查看关于您的docker安装的更多详细信息： 
 ```
 $ docker info
...
> With Docker, scaling your application is a matter of spinning up new executables, not running heavy VM hosts. 
 ...
 使用Docker，扩展应用程序只要一个新的可执行文件解决，而不要运行大量VM主机。 

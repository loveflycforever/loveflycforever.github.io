...
---
 ...
 - 找码头工人作曲。在DockerforMac和DockerforWindows上，它是预先安装的，所以你很好的选择。在Linux系统上，您需要直接安装它。在没有超级V的前Windows 10系统上，使用Docker工具箱.。阅读第一部分的方向。在第2部分中了解如何创建容器。确保您已经发布了您创建的FriendlyHello映像，方法是将其推送到注册表中。我们在这里使用共享的图像。确保映像作为已部署的容器工作。运行此命令，在您的信息中插入用户名、回购和标签：Dockerrun 80：80用户名/repo：tag，然后访问http：//localhost/。 
 ...
 在第3部分中，我们扩展了我们的应用程序并启用了负载平衡。要做到这一点，我们必须在分布式应用程序的层次结构中提升一个级别：服务。 
 
 Stack Services (you are here) Container (covered in part 2 ) 
 
 堆叠服务（您在此处）容器（第2部分包含）<br/> 

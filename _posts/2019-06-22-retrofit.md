...
它可以将 HTTP API 以 Java 接口的方式定义，向远程Web服务器发出同步或异步HTTP请求。
...
> 绝对地址和相对地址的区别：
“/users/132/repos”是绝对地址，地址前的“/”指向根目录，所以它是相对于根目录的地址，
"users/132/repos"是相对地址，即相对当前目录的地址。
...
然后，是指定请求体（Request Body）的注解`@Body`。可以定义一个对象作为请求体。不过，需要指定转换器用来转换对象，默认使用 RequestBody 这个类。
...
然后，是提交表单所需要的注解，`@FormUrlEncoded`、`@Field`。
...
如果是 Multipart 类型的表单请求，还会用到`@Multipart`、`@Part`。
...
Retrofit 提供合理的默认设置，同时也允许自定义配置。其中主要说明的就是转换器（Converters）。

默认情况下，Retrofit 只能将响应体反序列化为 OkHttp 包的 ResponseBody 类型，
并且`@Body`注解只能接受 RequestBody 类型。
但是，可以通过添加转换器的方式来支持其他类型。

Retrofit 根据以下 6 种流行的序列化库模块，提供了对应的转换器模块：
...
此时，再回关于 Retrofit 的简述，其中显而易见的关键字为`类型安全`、`HTTP 客户端`。

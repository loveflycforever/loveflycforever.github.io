...
![用户操作-行为分析-推断结论](../../img/用户操作-行为分析-推断结论.svg)
...
埋点需求一般有两个来源：_产品需求的衡量指标_和_业务部门的分析需求_。

通过这个步骤，我们可以知道收集数据的目的是什么。

# 3.3.3.2 选取衡量指标

明确了需求之后，我们就需要选取能够衡量需求效果的数据指标，比如页面转化率、功能留存率、访问人数、访问频次分布等等。
...
1. 确定收集哪些数据，当指标拆解成不能再拆的微小单元，那这个微小单元就是需要通过埋点收集的数据了，比如，“访问页面的独立用户数”。
...
埋点属性是决定分析深度和广度的关键因素，关于如何设计出完善的埋点属性，经典的思维模型就是5W1H，包括原因（Why ）、人物（Who）、时间（When）、地点（Where）、行为（What）、方式（How）。

# 3.3.4.1 5W1H分析法

记录用户因为什么在什么时间什么地点使用什么方式产生了什么样的行为，围绕5W1H完善埋点属性的设计，并撰写埋点文档。除了支撑数据指标的计算需求外，还要能够支撑实际工作中可能出现的分析需求。

# 3.3.4.2 WHY 原因

设计的缘由，一般来说就是支撑着这个指标或者这个埋点的主要原因，明确原因，可以极大减少不必要的需求和埋点，节约资源和开发成本。

# 3.3.4.3 WHO 人物

参与事件的用户，一般使用开发过程中对用户定义的唯一ID，包含用户的设备ID、UserID等非敏感信息。对用户的姓名、手机号、身份证号码等敏感信息不建议直接采集，如必须采集可采用脱敏的方式进行。
...
记录行为发生的时间，常见标准的的时间戳以外，还可以使用服务端的Session或登录序号等。记录值将用于区分用户的登录次数，界定活跃次数和行为归属。
...
事件是指具体的行为，属性是指行为相关的一些信息，如`商品下单`是`事件`，`商品ID`、`期望收货时间`、`快递方式`、`商品退货`、`商品ID`、`退货原因`、`退货价格`、`商品是否已寄回`是`属性`
...
更高级的可视化应用有很多，比如有动线系统。

动线起源于商业地产：在商场中，某一个顾客从入口到逛街到购买到出口的全流程的行动线路，就叫顾客动线。

依据对动线的分析，商场的每一个门店、每一个楼梯、每一个收银台的位置设置都有讲究的，能够最大化用户体验，最大化购物转化率。

同样，在APP中用户也存在一定动线。比如用户同样完成一个购买行为，可能是通过“列表页 - 详情页 - 购物车 - 付款”的动线，也可能是通过”活动页 - 详情页 - 直接下单 - 付款”的动线。

如果能够把这些动线通过可视化的方式展示出来，就可以看出动线之间的关系，以及每条动线的优劣势，从而跳脱出现有功能，而从整体规划这个流程的最优动线。
...
数据埋点通常不会一步搞定，第一次埋点会在产品的第一次上线时，通常会埋PC&Web端会统计产品的PV/UV，注册量，主要流程页面之间的转化率、日活人数等等。而移动端除此之外还要统计产品在应用商店的下载量。

第二次埋点会根据产品目标及上线后的问题进行分析。比如，当发现产品首页的UV很高，注册量却非常低，就需要分析出用户在首页的行为，如30%的用户退出了产品，60%的用户进入了注册页，但只有1%的用户注册了该产品。
...
__初级的数据埋点：__在产品流程关键部位植入相关统计代码，用来追踪每次用户的行为，统计关键流程的使用程度。

__中级的数据埋点：__在产品中植入多段代码追踪用户连续行为，建立用户模型来具体化用户在使用产品中的操作行为。

__高级的数据埋点：__与研发及数据分析师团队合作，通过数据埋点还原出用户画像及用户行为，建立数据分析后台，通过数据分析、优化产品。

# 四、事件模型

埋点体系中一个非常重要的角度是埋点模型，它决定了我们以什么样的架构收集用户行为信息。

埋点模型反应的是数据采集的世界观，即用什么样的视角去理解用户行为。

目前主流的埋点模型是“事件模型”。所谓的“事件模型”，其实是“事件 – 用户模型”，简单理解，我们把用户的行为记录为事件和用户，分别存在事件表和用户表中，事件表中记录“谁在什么时间，什么地点，以什么样的方式，做了一件什么样的事”，用户表里面记录了某个用户有什么样的属性特征，比如年龄、性别等。
...
- 抽象能力强，能全面、精细地描述的用户行为

- 可打通业务数据和行为数据，实现更精细的用户行为分析
...
“测试样本中有A类样本90个，B 类样本10个。分类器C1把所有的测试样本都分成了A类，分类器C2把A类的90个样本分对了70个，B类的10个样本分对了5个。则C1的分类精度为 90%，C2的分类精度为75%。但是，显然C2更有用些。”

需要的领悟是，对于衡量的准确精度来说，我们的标准要在统一的尺度下。

> ROC曲线，接受者操作特性曲线（receiver operating characteristic curve），又称为感受性曲线（sensitivity curve）。得此名的原因在于曲线上各点反映着相同的感受性，它们都是对同一信号刺激的反应，只不过是在几种不同的判定标准下所得的结果而已。接受者操作特性曲线就是以虚惊概率为横轴，击中概率为纵轴所组成的坐标图，和被试在特定刺激条件下由于采用不同的判断标准得出的不同结果画出的曲线。
>
> AUC（area under curve）即ROC曲线下的面积。（随机给定一个正样本和一个负样本，分类器输出该正样本为正的那个概率值 比 分类器输出该负样本为正的那个概率值 要大的可能性）

以页面访问率举例，我们可以通过两方面来统计，一种是页面统计，另一种是行为统计。

__页面统计页面访问率__

有的页面是存在唯一的主次关系，意思是B页面仅能从A页面进入。此时，我们结合两个页面的访问数值，就能得到A页面到B页面的转化率。并以此判断两个页面是否存在可优化空间。数据里所隐藏的优化空间，往往是存在多种可能性。
...
（1）A页面转化部分设计有问题，就像文章的阅读数和标题息息相关，可能需要优化入口的设计。

（2）B页面留存有问题，无法让用户产生二次访问行为，以及无法让用户形成更高频率的访问。

对于 有多个入口的页面，页面路径的分析方法就没有作用了，此时，就需要借助行为统计。

__行为统计页面访问率__

一些基础的功能，往往被多个页面应用，也能通过两个以上的页面进入，也就是我们所熟知的“多个入口”的概念。此时，我们可以借助 指定入口页的访问人数，入口按钮的点击人数，来判断该页面的转化率。

页面可以通过ACD三个页面点击对应按钮进入，想要知道A页面到B页面的转化率是多少。若A页面访问人数是100，按钮点击人数是30，而B页面访问人数是40，问题就变成了，在A页面点击按钮的点击率，这种分析方法，更多的是侧重对入口所在页的分析，通常提到的转化率，也是指类似的分析方法，比较典型的是广告转化率提升的设计方法。

此类型产品，几乎不考虑“复购”问题，其追求的目标是最大限度，在不影响原有用户体验的基础之上，提高用户点击广告的几率。与之对应的产品设计方法，更多的在于视觉的设计，类似加粗，图片，特殊形状，诱惑性的文案等。

# 5.2 统计的内容

统计广告位点击次数，但是广告位点击计数是要做些什么呢？

区分三个要素：用户、广告位、广告。用户泛指用户的行为，广告位是在网页或者APP中的广告位置，广告是特指某一类吸引客流的宣传。

作为一个用户，进入网站在某个广告位停留，点开某个广告，然后进行浏览，通过这一条行为路径，怎么判断哪一个位置投放什么广告更好。

1. 相同的广告放在不同的位置，用户点击率高的必然是一个较好的位置。比如，有的广告位置是首页，有的是支付完成页，这个有助于分析如何有效植入广告。

2. 对于同一个广告位置，不同广告的点击率，可以研究什么？对比来研究大众的倾向点，对哪些产品更有兴趣，是一种对用户心理诉求的追踪。

3. 对于同一个广告的点击情况，我们就要分析用户群体特征，是男性多，还是女性多，年龄段集中在什么层次。这说明活动的受众群体。

4. 投放的最终目标是高转化率。需要知道每个广告的转化率，某个活动的广告点击量多，不代表这个活动就很火。

5. 回到需求的起初，当大量用户进入到页面，有多少人点开了广告页，这是到下一步分析转化的前提，与后面的数据是一个转化漏斗。
6. 为了有效评估宣传推广的价值，还要区分这个用户是由外部宣传吸引而来，还是本来就已经在使用产品。

总的来说，多少人来到页面，多少人点开了广告的门，有着非常多的门道。这样区分后的数据分析的才有价值，具体数据的规则可由不同的产品和不同的业务来决定。
...
按照开发方式划分为手工埋点、可视化埋点和无埋点。按照埋点位置划分为客户端埋点和服务端埋点。

> 客户端是记录用户在客户端的使用数据，包括但不限于客户端信息、位置信息等等，而服务端主要是记录程序接口的调用情况，例如接口访问次数，接口返回各个状态次数的统计等等。

数据采集方式的选择是基于业务场景的，没有一种数据采集方式可以解决一切问题，各种方式依然有其短板，在使用时要了解这些弊端，不同的业务场景下应该选取不同的数据采集方式，并判断这种采集方式是否可以支持业务需求。

# 7.1 手工埋点&可视化埋点&无埋点的优缺点

# 7.1.1 手工埋点

最常见的一种是代码埋点，即手工埋点，是研发将用于监听用户行为的代码提前手动埋到触发该事件的代码中。

__优点：__

方面即可以自定义进行数据采集，对数据分析更精细化，数据采集能力较强，只要是数据需求合理，原则上都可以满足并最终统计到想要的数据。

__缺点：__

受到整个项目发版流程的限制，每次埋点的升级都要发版支持。同时，新功能上线后如果埋点没有埋上或者埋点信息异常，如非极其重要的数据也只能等待下一个发版周期。

开发成本及沟通成本高，具体来说，项目工程量大，需要产品给研发提埋点需求，研发需要在代码中写入监听的代码，产品上线前还需要验证。
...
当一种操作方式有它存在的局限性，随之就会有其他更便捷高效的方式出现，一些第三方数据平台或诸如BAT这种互联网公司已经想出一种可视化埋点方式。指通过设备连接用户行为分析工具的数据接入管理界面，对可交互且交互后有效果的页面元素（如：图片、按钮、链接等），直接在界面上进行圈选定义事件实现数据埋点，下发采集代码生效回数的埋点方式，来追踪用户行为。

__优点：__

极大的降低了人力成本，相较于手工埋点产品、开发、测试、运营多方参与的的过程，可视化埋点只需要运营和产品对接平台即可。

相比于手动埋点更新困难，埋点成本高的问题，可视化埋点类似于前端网页的形式，优化了移动运营中数据采集的流程，能够支持产品运营随时调整埋点，无需再走发版流程，直接把配置结果推入到前端，数据采集流程更简化，也更方便产品的迭代，实时交互降低人力成本和出错成本。

__缺点：__

既要轻量级就没法完全自定义，导致数据精准度不高、数据维度单一，对于一些基本的记录某个页面的展现和按钮点击等可见元素可能没问题，但是面对一些复杂的数据需求时这种方式便没辙了，比如需要区分来源的数据需求，带各种参数的需求，需要和后端交互的数据需求就不太适合可视化埋点方式，就是能够知道用户点击了购买，但不知道购买了什么。

__适用场景：__

可视化埋点虽然很美好但仅限于一些简单的纯客户端埋点统计，主要分析点击事件。
...
还有一种无埋点方式，也叫全埋点，即事先尽可能收集所有控件的操作数据，然后再通过界面配置哪些数据需要在系统里面进行分析。当然，它并不是真正的不需要写代码，而是前端通过加载一段定义好的代码，自动采集全部事件并上报所有的数据。

__优点：__

能够呈现用户行为的每一次点击、每一次跳转、每一次登录等全量、实时用户行为数据，并通过“圈选”来获取需要使用的事件。圈选数据不需要组织协作，产品经理不依赖任何人就能采集到数据，并马上在分析工具中使用，所见即所得。

解决了手工埋点和可视化埋点共同的一个缺点即数据回溯，较好的解决了忘了“埋点”这个痛点。

__缺点：__

继承了可视化埋点的缺点，所有数据维度单一（如点击、加载、刷新）、数据准确性不高、上传数据多，消耗服务器资源多，同时也会受开发框架、开发规范以及开发能力的影响。

无埋点采集的数据是通过界面位置和上下级关系来标示自己的，一旦界面发生较大变化，会导致数据无法持续采集，需要重新圈选。

__适用场景：__

产品功能相对简单，主要分析活动页、登录页、关键页面设计体验。
...
1. 做了一场运营活动，没有开发埋点功能的资源。
2. 想衡量交互细节，而需要查看的交互细节非常多。例如搜索流程。
3. 想查看一个用户在访问时的一切行为轨迹，探索用户使用产品场景。
4. 每周发版，衡量发版的效果。
5. 要做一个分析，埋点和积累数据来不及，需要尽快产出结论。
6. 新上线的功能，有一个重要的元素忘了埋点，需要及时补充。
...
1. 深度业务分析：比如不同 SKU 的购买转化分析。

2. 归因分析：比如产品不同入口的带来的销售额分析。
...
两类场景不是互斥的。一个用无埋点采集方式就能解决的问题，若使用埋点，既不会立刻看到数据，也会浪费公司工程资源；一个埋点采集才能解决的问题，用了无埋点可能会导致数据不稳定，业务维度少等问题。
...
前端埋点的优势是可以事无巨细的统计到用户的行为数据，但为了性能考虑，并不会实时上报数据，所以注定了前端埋点的数据在时效性和准确性上不会做到100%精准无误。

__优点：__

在于搜集页面展示、点击行为，不需要请求服务器的数据。

__缺点：__

无网络时数据不完整、实时性有延迟，会存在漏报的情况。当需要改变埋点时，必须更新版本。

同时，可能会影响性能，受限于开发者技术，影响用户体验（微乎其微的那种）。

相比于后端数据量少，脱离用户使用层的数据缺失。

# 7.2.2 服务端埋点

统计某款社区产品每天的用户发帖记录，除了可以直接在客户端埋点，也可以在服务端接口的处埋点。

__优点：__

在于实时性好，数据准确；即使发生变更，可以透明切换，所以相较而下成本低；而且能够收集不在客户端内发生的行为，比如，统计从其他APP引流的安装量，这些数据只请求了服务器。

__缺点：__

明显的问题的就是不能收集不需要请求服务器的数据，所以不易验证前端页面设计效果，诸多交互行为记录，也会出现数据不足，比如页面停留时长等数据，需要前端专门提供。
...
频次事件就是普通的统计事件。例如，按钮控件的点击事件、页面的展示事件，可以通过事件的频次，判断对应功能的价值。

__状态事件：__

状态事件是指事件的最终状态。例如，开关的开启与关闭，在指定的时间段，只关心最终状态，不关心开关多少次。对于状态事件，一般用`用户属性`来表示。但是对于很多统计，用户属性存在上限，所以对于颗粒度很小，并且操作可能非常频繁的状态事件来说，也可以通过频次事件的数值进行粗略判断。例如，音乐应用中，通常会使用该音乐的收藏按钮的点击次数（频次事件）推断被收藏的数量（状态事件）来判断音乐的受欢迎程度。

# 9.1.2 具体事件与抽象事件

__具体事件：__

指的是某个具体操作，例如，收藏按钮的点击、活动页面的展示、通知弹窗的弹出等。

__抽象事件：__

往往指一类事件的集合，例如，计算模块之间的漏斗转化率，往往会用到抽象事件。

# 9.1.3 主动事件与被动事件

__主动事件：__ 

主动事件代表了用户的主动意愿，需要用户主动触发，例如，点击按钮，或者切换Tab，对于用户行为分析具有很大的参考意义。

__被动事件：__

往往是用户操作的附加结果，不能表明用户的使用意愿。例如，用户点击返回键从当前页面A回退到上一个页面B，此时页面B的展示并不一定是用户真的想要进入，只是退出页面A的必经路径。这类事件对于用户的行为分析意义不是很大，但是对于其他维度的分析有着一定的参考价值，例如广告展示等。
...
页面事件通常是指页面的各种维度信息的统计，可能包含有页面跳转关系。比如，页面浏览PV、UV（统计某个页面被多少人访问了多少次）。

同时，也可以监控到用户在某个页面停留的时长。部分产品希望用户在某个页面停留的时间越长越好，追求停留时间的产品，典型的便是信息流产品。这表示用户正在持续的进行阅读，停留的时间越长，表示内容对用户的吸引力越高，这样才能产生持续的阅读行为。
...
用户在应用内的每一次点击行为，都可以记为一次点击事件，不论点击的结果是否成功。点击事件包含单击、双击、拖拽等交互事件，一般通过点击事件，可以拿到点击PV、UV。

比如，当在购物车页面时，发现自己之前添加了一把键盘，但是没有今天看到的喜欢，于是在购物车页面选取那个键盘进行了删除。这个时候，就需要上报事件埋点，并携带被删除商品ID及加入购物车时间等参数。

# 9.2.3 曝光事件（Expose）

曝光事件是为了统计应用内的某些局部区域是否被用户有效浏览。比如推荐区域，某个按钮，首页焦点等等，要根据不同需求自定义。比如成功打开1次页面计1次，或者页面停留超过10s计1次等。
...
记录用户在文本框输入的任何信息和动作。比如，与人聊天时时，反复斟酌、踌躇犹豫的句读与辞藻，在此处都能记录得明明白白。连带初始输入，删除，撤回等等，还有总输入时间，都会被记录且细化到毫秒级。

> 所以不要再质疑企业能获取你的信息，数据安全真的全靠自觉。
...
![业务分解-分析指标-事件设计-属性设计](../img/业务分解-分析指标-事件设计-属性设计.svg)
...
上报/采集时间、来源业务线、来源终端（ APP/微信/网页）、来源渠道、经纬度，等等。这个业务侧可以根据业务需求增加多个，个性化很强的上报，放在同一个json格式的字段即可。
...
_pt/_pi/_pm系列：定位页面和模块，可用于不同业务线的分析，例如首页、要问频道、正文页等_
...
事件转化漏斗：即按照一定的事件顺序，依次统计各个事件之间的转化率，如我们可以对登录注册中的一些关键步骤进行事件漏斗分析，如输入手机号码，获取验证码、输入验证码等，以登录过程为例，其漏斗可设置为：输入手机号码->获取验证码->输入验证码->点击登录按钮，即由4个事件组成的漏斗。
...
一个行为的发生要经历“事件触发（前端） → 数据入库（后端） → 事件发生（前端）”的过程，以电商购买这个操作为例，“点击购买按钮（事件触发） → 后端验证库存等信息后返给前端结果（数据入库） → 跳转到下单页（事件发生）”。
...
还是以商品下单流程为例，想验证直播带货的能力，就需要区分直播间下单和普通浏览下单，有以下两种方案：
...
# 10.1.4 抽离出公共属性
 ...
 埋点信息中有些属性是很多事件中都会用到的，比如用户身份是否vip、省份，以及使用的设备型号和操作系统，这些属性可以抽离出来作为公共属性，统一上报，不需要每个事件都单独上报这些属性。
